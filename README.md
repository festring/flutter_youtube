...
> 아래 3개의 파일에 안드로이드 빌드 관련 핵심 설정들이 담겨 있습니다.
...
- **`app/build.gradle`**
    ...
    앱 모듈의 빌드 설정 파일로, 안드로이드 SDK 버전, 의존성 라이브러리, 빌드 타입(디버그, 릴리스 등) 등을 정의합니다. 실제 APK 생성에 중요한 역할을 합니다.
    
- **`app/src/main/AndroidManifest.xml`**
    
    앱의 메타데이터를 정의하는 파일로, 앱 이름, 아이콘, 권한 설정, 액티비티 정의 등이 포함되어 있습니다. 앱의 핵심 정보가 여기에 기록됩니다.
    

## 📁 assets 폴더

> JavaScript 파일과 앱 아이콘 등의 원본파일이 담겨있습니다.
JS 코드가 유튜브 내에서 환경을 제어하고 동작을 관리합니다.
...
- **`check.js`**
    ...
    **※ 핵심 기능 ※**
    ...
    배속 조정 등 웹 **동작을 감지**하고 배열에 **기록**하는 기능을 담습니다.
...
- **`control.js`**
    ...
    **※ 핵심 기능 ※**
    ...
    서버에 설정된 시간 타점 기록을 기반으로 배속을 조정해주는 기능을 담고있습니다.
    해당 기능은 코드가 복잡하여, 세부 내용은 주석에 상세하게 담았습니다.
    
    **참고)**
    
    초기에는 서버와 통신하여 영상을 분석한 후, 해당 결과를 기반으로 배속 조정했습니다.
    해당 내용은 현 실험에서는 사용하지 않기로 결정되어 전부 주석 처리되었습니다.
    

- **`adblock.js`**
    
    유튜브 내의 광고를 스킵하는 기능을 담았습니다.
    광고 시간을 넘기는 것 이외에도 skip 버튼을 자동으로 클릭합니다. 
    
- **`dash.js`
※ 주의 : 현재 실험에는 필요없는 기능을 담고 있습니다.**
...
- **`main.js`**
    ...
    **※ 주의 : 이 파일은 이론상 필요 없는 파일입니다**
    
    근데 없으면 괜히 불안해서 못지우고 있습니다. 하하 
...
> 아래 2개의 파일에 IOS 빌드 관련 핵심 설정들이 담겨 있습니다.
이외에도 빌드에 문제가 발생하면, 문제가 되는 파일을 직접 찾아야합니다..
> 
- **`Runner/Info.plist`**
    
    iOS 앱의 구성 파일로, 앱 이름, 번들 식별자, 권한 설정, 환경설정 등이 정의되어 있습니다. iOS에서 앱이 어떻게 동작하는지를 제어합니다.
    
- **`Podfile`**
    
    CocoaPods 의존성 관리를 위한 파일로, iOS 앱에서 사용하는 외부 라이브러리나 플러그인을 정의합니다. `pod install` 명령어를 통해 이 파일을 기반으로 의존성을 설치합니다.
...
- **`main.dart`**
    ...
    **※ 핵심 기능 ※**
    ...
    Flutter 앱의 전체 구조, 라우팅, 위젯 트리의 시작점 등 모든 것들이 파일에 정의됩니다.
    자세한 설명은 코드에 주석처리를 해두었습니다.
    
- **`firebase_options.dart`**
    
    Firebase 서비스를 사용하는 앱에서 Firebase 초기화를 위한 설정이 저장된 파일입니다. Firebase 콘솔에서 다운로드한 설정 정보를 포함하며, 앱에서 Firebase 기능을 활성화합니다.
...
- **`pubspec.yaml`**
    ...
    Flutter 및 Dart 프로젝트의 **구성 정보와 의존성을 관리하는 핵심 파일**입니다.
...
> 현재는 @**hch2454@khu.ac.kr**의 firebase realtime database에 연결되어있습니다.
...
현재는 기존의 hch2454@ 서버에 연결해서 진행.(서버 키는 법은 보안상 직접 전달)
...
[1,[[10, 1.02],[15, 1.04],[20, 1.06],[30, 1.08],[40, 1.09],[50, 1.1]],"17o8zXEfMww"],
[1,[[20, 1.05],[25, 1.07],[30, 1.1],[40, 1.12],[50, 1.15],[60, 1.2]],"dV40PafQd3k"],
[1,[[10, 1]],"zotPeCofnRw"],
[1,[[2, 1.1],[20, 1.15],[40, 1.2],[50, 1.25]],"faRClqed55E"],
[1,[[6, 1.25],[25, 1.2],[30, 1.25],[40, 1.2],[50, 1.25],[60, 1.2]],"moFB-j5iY2E"],
[1,[[20, 1.05],[26, 1.08],[40, 1.1],[60, 1.11],[75, 1.12],[80, 1.15]],"zganb8Yqh3M"],
[1,[[10, 1]],"RidmoOUbZ5U"],
[1,[[20, 1.05],[26, 1.08],[40, 1.1],[60, 1.11],[75, 1.12],[80, 1.15]],"ZsEwZ4ym3OE"]
]
```
